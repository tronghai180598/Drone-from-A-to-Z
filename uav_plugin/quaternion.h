...
// Arduino compatibility: remove Printable base class for Gazebo
class Quaternion {
public:
	float w, x, y, z;
    ...
    // Roll (phi), Pitch (theta), Yaw (psi) theo quy ước aerospace chuẩn:
    // roll quanh X, pitch quanh Y, yaw quanh Z (Z-up)
    float getRoll() const {
    ...
    float getPitch() const {
        ...
        // clamp tránh NaN do sai số số học
    ...
    // fromEuler already defined above (line 38), removed duplicate
	...
	// PX4-style quaternion derivative: q_dot = 0.5 * q ⊗ [0, w]
	Quaternion derivative1(const Vector& w_body_rad_s) const {
		Quaternion v(0.0f, w_body_rad_s.x, w_body_rad_s.y, w_body_rad_s.z);
		Quaternion qdot = (*this) * v;
		qdot.w *= 0.5f;
		qdot.x *= 0.5f;
		qdot.y *= 0.5f;
		qdot.z *= 0.5f;
	...
	// Arduino printTo removed for Gazebo compatibility
};
